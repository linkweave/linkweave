...
// OIDC (Google) login requires a browser redirect to a fixed callback URL, which the desktop
// bundle (UC-052) cannot satisfy on a random localhost port. Hide it there; form auth remains.
...
<template>
  <AuthLayout :title="t('app.title')">
    <div class="space-y-6">
      <ButtonLw v-if="!isDesktop" class="w-full gap-2" @click="signInWithGoogle">
      ...
      <form @submit.prevent="onSubmit" class="space-y-4">
        ...
        <FormFieldLw :label="t('login.password')" for-id="login-password" :error="errors.password">
          <input
            id="login-password"
            v-model="password"
