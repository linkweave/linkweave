...
/**
 * JAX-RS {@link ParamConverterProvider} that binds path/query/form parameters of
 * type {@link ID} (e.g. {@code ID<BookmarkEntity>}) by parsing the UUID and
 * recovering the entity class from the parameterized type.
 */
@Provider
        ...
        if (type.getActualTypeArguments().length != 1) {
            throw new IllegalArgumentException(
                "This implementation expected ID to have exactly one type argument, e.g.: ID<SomeEntity>."
                    + " Actual declaration: " + genericType);
        }
