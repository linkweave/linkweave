...
/**
 * Hibernate {@link EnhancedUserType} that persists an {@link ID} as its UUID
 * string in a varchar column, recovering the concrete entity class from the
 * property's generic type information (with a fallback for envers audit tables).
 */
...
public class IDType implements EnhancedUserType<ID>, DynamicParameterizedType {
    ...
    public void setParameterValues(Properties parameters) {
        ...
        if (propertyIsOnEnversAuditTable) {
            ...
            entityClass = parseEntityClassFromEntityName(entity);
            return;
        }
    ...
    private Class<?> parseEntityClassFromEntityName(String className) {
        try {
    ...
    public int getSqlType() {
        return SqlTypes.VARCHAR; // sqlite does not have a uuid type
    }
