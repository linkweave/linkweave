#!/usr/bin/env bash
#
# Builds the Chainlink desktop bundle (UC-052). macOS-first; see docs/desktop-app.md
...
# Stage 1 — build the SPA with the desktop flag (hides the OIDC button; #2).
log "1/6 build SPA (VITE_DESKTOP=true)"
( cd frontend && pnpm install --frozen-lockfile && VITE_DESKTOP=true pnpm run build )

# Stage 2 — package the backend with the desktop profile: root-path=/, REST under /api, OIDC
# compiled out (#4c). Same fast-jar layout as the Docker build, different build-time config.
log "2/6 package backend (quarkus.profile=desktop)"
( cd api && ./mvnw -q -DskipTests package -Dquarkus.profile=desktop )

# Stage 3 — smoke-test: the packaged jar must actually boot under the desktop profile and answer
...
log "3/6 smoke-test backend boots (quarkus.profile=desktop)"
SMOKE_PORT=18123
...
# Stage 4 — stage payloads as Tauri resources. The SPA is NOT baked into the jar; it ships as a
...
if [ "$BUNDLE_JRE" = true ]; then
  echo "jlinking a trimmed Java 25 runtime…"
  ...
  "$JAVA_HOME_DIR/bin/jlink" --add-modules ALL-MODULE-PATH --module-path "$JAVA_HOME_DIR/jmods" \
...
# Stage 5 — bundle the .app/.dmg.
log "5/6 tauri build"
( cd desktop && cargo tauri build )

# Stage 6 — collect the artifact in one predictable place. Clear first: a prior bundle's runtime
# has read-only legal/ files that a plain `cp` over them can't overwrite (EACCES). chmod makes the
# stale tree removable.
log "6/6 collect artifact"
...
echo
echo "Done. Artifacts in $REPO_ROOT/desktop/dist/"
