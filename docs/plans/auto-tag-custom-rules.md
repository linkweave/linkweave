...
No special concern — entity sits in the existing `bookmark` package (it's tightly coupled to tag resolution). If `linkweave-api` ArchUnit complains about location, move to a dedicated `org.linkweave.api.autotag` package.
