...
| FR-042 | Adopt Form Library with Zod Schemas | As a Developer, I want to replace manual form validation with a form library (e.g. VeeValidate or Formkit) backed by Zod schemas so that form handling is consistent, type-safe, and less error-prone across the frontend. Schemas should be auto-derived from the generated API types where possible. | Medium   | Open |
